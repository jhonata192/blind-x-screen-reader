...
def module_interface_name(module_name: str) -> str:
    ...
    return f"org.gnome.BlindX1.{module_name}"


def list_module_names(bus: SessionMessageBus) -> list[str]:
    """Returns sorted module names registered under /org/gnome/BlindX1/Service."""
...
def _bus(blind_x: Any) -> SessionMessageBus:
    """Returns a session-scoped bus connection. The blind_x fixture ensures Blind-X is up."""
...
def _dbus_service_proxy(bus: SessionMessageBus) -> Any:
    """Returns a D-Bus proxy for the Blind-X service root."""
