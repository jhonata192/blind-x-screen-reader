...
"""Shared helpers for building the isolated environment Blind-X-launching fixtures use."""
...
def build_sandbox_env(sandbox: Path) -> dict[str, str]:
    """Builds an isolated environment for Blind-X; points Blind-X's XDG dirs into the sandbox."""

    env = os.environ.copy()
    # XDG_RUNTIME_DIR is owned by the wrapper (it hosts the AT-SPI sockets);
    # only the Blind-X-specific XDG dirs are sandboxed here.
    for subdir, key in (
        ("data", "XDG_DATA_HOME"),
