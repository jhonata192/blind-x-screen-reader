...
These tests exercise Blind-X's GSettings infrastructure against real Gio.Settings
...
class TestRegistryLookup:
    ...
    def test_enum_lookup_returns_string_nick(
        ...
        result = gsettings_registry.layered_lookup(
            ...
            "verbosity-level",
            "",
            genum="org.gnome.BlindX.VerbosityLevel",
        )
        assert result == "brief"
...
class TestProfileOperations:
    ...
    def test_reset_profile_clears_all_user_keys(
        ...
        gs_after = Gio.Settings.new_with_path(
            handle.get_schema_id(), "/org/gnome/blind-x/doomed/speech/"
        )
    ...
    def test_rename_profile_moves_keys(
        ...
        gs_new = Gio.Settings.new_with_path(
            handle.get_schema_id(), "/org/gnome/blind-x/new-name/speech/"
        )
        assert gs_new.get_boolean("enable") is False

        gs_old = Gio.Settings.new_with_path(
            handle.get_schema_id(), "/org/gnome/blind-x/old-name/speech/"
        )
