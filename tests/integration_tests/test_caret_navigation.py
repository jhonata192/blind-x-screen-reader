...
"""Exercise Blind-X's announcements for caret navigation and selection in a TextView."""
...
from blind_x.output_reader import BrailleRecord, SpeechRecord
...
def test_caret_navigation_presentation(gtk3_text_view: NativeAppSession) -> None:
    """Blind-X's announcements as the caret moves via arrows, word/line jumps, buffer endpoints."""
...
def test_caret_selection_presentation(gtk3_text_view: NativeAppSession) -> None:
    """Blind-X's announcements as selection changes by line, word, char, jumps, and select-all."""
    ...
    session.blind_x.press_blind_x_key(keyboard.KEYSYM_UP, extra_modifiers=[keyboard.KEYSYM_SHIFT_L])
    assert _capture(session) == (
        [
