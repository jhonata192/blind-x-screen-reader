...
class TestFlatReviewPresenter:
    ...
    def _setup_dependencies(self, test_context: OrcaTestContext) -> dict[str, MagicMock]:
        ...
        braille_mock = essential_modules["blind_x.braille"]
        ...
        ax_event_synthesizer_mock = essential_modules["blind_x.ax_event_synthesizer"]
        ...
        speech_verbosity_mock = essential_modules["blind_x.speech_presenter"]
        ...
        platform_mock = essential_modules["blind_x.orca_platform"]
    ...
    def test_init(self, test_context: OrcaTestContext) -> None:
        ...
        from blind_x.flat_review_presenter import FlatReviewPresenter

        mock_controller = test_context.Mock()
        test_context.patch(
            "blind_x.extension.dbus_service.get_remote_controller",
    ...
    def test_get_or_create_context_creates_new_unrestricted(
        ...
        essential_modules["blind_x.flat_review"].Context.assert_called_with(script_mock)
    ...
    def test_get_or_create_context_creates_new_restricted(
        ...
        assert context is not None
        assert presenter._context is not None
        essential_modules["blind_x.flat_review"].Context.assert_called_with(script_mock, root=root_obj)
    ...
    def test_get_or_create_context_returns_existing(self, test_context: OrcaTestContext) -> None:
        ...
        assert essential_modules["blind_x.flat_review"].Context.call_count == 0
    ...
    def test_start_scenarios(
        ...
        if has_context:
            essential_modules["blind_x.flat_review"].Context.assert_not_called()
        else:
            assert presenter._context is not None
            essential_modules["blind_x.flat_review"].Context.assert_called_once()
    ...
    def test_navigation_methods(
        ...
        getattr(context_mock, context_method).assert_called_once_with(
            getattr(essential_modules["blind_x.flat_review"].Context, navigation_target),
        )
    ...
    def test_go_bottom_left(self, test_context: OrcaTestContext) -> None:
        ...
        context_mock.go_to_end_of.assert_called_with(
            essential_modules["blind_x.flat_review"].Context.WINDOW,
        )
        context_mock.go_to_start_of.assert_called_once_with(
            essential_modules["blind_x.flat_review"].Context.LINE,
        )
        mock_present_line.assert_called_once_with(script_mock, event_mock)
        assert result is True
    ...
    def test_restricted_vs_unrestricted_context_creation(
        ...
        assert essential_modules["blind_x.flat_review"].Context.call_count >= 2
    ...
    def test_context_caching_behavior(self, test_context: OrcaTestContext) -> None:
        ...
        essential_modules["blind_x.flat_review"].Context.reset_mock()
        ...
        assert essential_modules["blind_x.flat_review"].Context.call_count == 1
