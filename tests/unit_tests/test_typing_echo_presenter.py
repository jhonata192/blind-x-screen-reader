...
class TestTypingEchoPresenter:
    ...
    def _setup_essential_mocks(test_context: OrcaTestContext, essential_modules: dict) -> None:
        ...
        essential_modules["blind_x.debug"].LEVEL_SEVERE = 1000
        essential_modules["blind_x.debug"].debugLevel = 1000
    ...
    def _setup_presenter(self, test_context: OrcaTestContext):
        ...
        self._setup_cmdnames(essential_modules["blind_x.cmdnames"])
        self._setup_essential_mocks(test_context, essential_modules)
        self._setup_guilabels(essential_modules["blind_x.guilabels"])

        self._setup_atspi_patches(test_context)

        from blind_x import gsettings_registry
        from blind_x.typing_echo_presenter import TypingEchoPresenter
    ...
    def test_locking_keys_presented_getter_and_setter(self, test_context: OrcaTestContext) -> None:
        ...
        speech_presenter_patch = test_context.patch("blind_x.speech_presenter.get_presenter")
    ...
    def test_cycle_key_echo_with_script_presentation(self, test_context: OrcaTestContext) -> None:
        ...
        from blind_x import presentation_manager
    ...
    def test_should_echo_keyboard_event_basic_cases(self, test_context: OrcaTestContext) -> None:
        ...
        presenter.set_key_echo_enabled(False)
        event_mock.is_blind_x_modifier.return_value = False
        event_mock.is_alt_control_or_blind_x_modified.return_value = False
    ...
    def test_should_echo_keyboard_event_orca_modifier(self, test_context: OrcaTestContext) -> None:
        """Test should_echo_keyboard_event for Blind-X modifier keys."""
        presenter = self._setup_presenter(test_context)
        ...
        event_mock.is_pressed_key.return_value = True
        event_mock.is_blind_x_modifier.return_value = True
    ...
    def test_should_echo_keyboard_event_modified_keys(self, test_context: OrcaTestContext) -> None:
        ...
        event_mock.is_alt_control_or_blind_x_modified.return_value = True
        assert presenter.should_echo_keyboard_event(event_mock) is False
    ...
    def test_should_echo_keyboard_event_password_text_obscuring(
        ...
        test_context.patch("blind_x.ax_utilities.AXUtilities.is_password_text", return_value=True)
        assert presenter.should_echo_keyboard_event(event_mock) is False

        # Test with password text that should not be obscured - should echo alphabetic
        event_mock.should_obscure.return_value = False
        test_context.patch("blind_x.ax_utilities.AXUtilities.is_password_text", return_value=False)
        assert presenter.should_echo_keyboard_event(event_mock) is True
    ...
    def test_is_character_echoable(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.ax_utilities.AXUtilities.is_password_text", return_value=False)
        test_context.patch("blind_x.ax_utilities.AXUtilities.is_editable", return_value=True)
        test_context.patch("blind_x.ax_utilities.AXUtilities.is_terminal", return_value=False)
        assert presenter.is_character_echoable(event_mock) is True

        test_context.patch("blind_x.ax_utilities.AXUtilities.is_editable", return_value=False)
        test_context.patch("blind_x.ax_utilities.AXUtilities.is_terminal", return_value=True)
        assert presenter.is_character_echoable(event_mock) is True
    ...
    def test_echo_previous_word(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.ax_text.AXText.get_caret_offset", return_value=5)
        test_context.patch("blind_x.ax_text.AXText.get_character_count", return_value=10)

        test_context.patch("blind_x.ax_text.AXText.get_caret_offset", return_value=0)
        assert presenter.echo_previous_word(obj_mock) is False

        test_context.patch("blind_x.ax_text.AXText.get_caret_offset", return_value=5)
        char_mock_1 = test_context.patch("blind_x.ax_text.AXText.get_character_at_offset")
        char_mock_1.side_effect = [("a", 4, 5), ("b", 3, 4)]
        assert presenter.echo_previous_word(obj_mock) is False

        char_mock_2 = test_context.patch("blind_x.ax_text.AXText.get_character_at_offset")
        char_mock_2.side_effect = [(" ", 4, 5), (" ", 3, 4)]
        assert presenter.echo_previous_word(obj_mock) is False

        char_mock_3 = test_context.patch("blind_x.ax_text.AXText.get_character_at_offset")
        char_mock_3.side_effect = [(" ", 4, 5), ("a", 3, 4)]
        test_context.patch("blind_x.ax_text.AXText.get_word_at_offset", return_value=("hello", 0, 5))
        ...
        char_mock_4 = test_context.patch("blind_x.ax_text.AXText.get_character_at_offset")
        char_mock_4.side_effect = [(" ", 4, 5), ("a", 3, 4)]
        test_context.patch("blind_x.ax_text.AXText.get_word_at_offset", return_value=("", 0, 0))
        assert presenter.echo_previous_word(obj_mock) is False
    ...
    def test_echo_previous_sentence(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.ax_text.AXText.get_caret_offset", return_value=10)
        ...
        test_context.patch(
            "blind_x.ax_text.AXText.get_sentence_at_offset",
        ...
        test_context.patch("blind_x.ax_text.AXText.get_sentence_at_offset", return_value=("", 0, 0))
        assert presenter.echo_previous_sentence(obj_mock) is False
    ...
    def test_getter_returns_dconf_value_when_available(
        ...
        from blind_x import gsettings_registry
        from blind_x.gsettings_registry import GSettingsSchemaHandle
    ...
    def test_get_setting_logs_dconf_layer(self, test_context: OrcaTestContext) -> None:
        ...
        from blind_x import debug as debug_mock
        from blind_x import gsettings_registry
        from blind_x.gsettings_registry import GSettingsSchemaHandle
