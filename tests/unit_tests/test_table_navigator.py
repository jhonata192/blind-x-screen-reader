...
class TestTableNavigator:
    ...
    def _setup_dependencies(self, test_context: OrcaTestContext) -> dict[str, MagicMock]:
        ...
        cmdnames = essential_modules["blind_x.cmdnames"]
        ...
        essential_modules["blind_x.blind_x_i18n"]._ = lambda x: x
    ...
    def test_init(self, test_context: OrcaTestContext) -> None:
        ...
        essential_modules["blind_x.dbus_service"].get_remote_controller.return_value = mock_controller
        ...
        test_context.patch(
            "blind_x.table_navigator.keybindings.KeyBindings",
            new=mock_keybindings_class,
        )
        from blind_x import command_manager
        from blind_x.table_navigator import TableNavigator
    ...
    def test_last_input_event_was_navigation_command_none(
        ...
        essential_modules["blind_x.input_event_manager"].get_manager.return_value = mock_manager
    ...
    def test_toggle_enabled(
        ...
        guilabels_mock = essential_modules["blind_x.guilabels"]
        ...
        assert result is True
        expected_message = getattr(essential_modules["blind_x.messages"], expected_message_attr)
    ...
    def test_is_blank(
        ...
        essential_modules = self._setup_dependencies(test_context)
        test_context.patch(
            "blind_x.table_navigator.AXUtilities.is_focusable",
            return_value=is_focusable,
        )
        test_context.patch(
            "blind_x.table_navigator.AXObject.get_name",
            return_value="name" if has_name else "",
        )
        test_context.patch(
            "blind_x.table_navigator.AXObject.get_child_count",
            return_value=1 if has_children else 0,
        )
        test_context.patch(
            "blind_x.table_navigator.AXUtilities.is_whitespace_or_empty",
            return_value=is_whitespace,
        )
        if has_children:
            mock_child = test_context.Mock()
            test_context.patch(
                "blind_x.table_navigator.AXObject.iter_children",
            ...
            test_context.patch("blind_x.table_navigator.AXObject.iter_children", return_value=[])
        mock_controller = test_context.Mock()
        essential_modules["blind_x.dbus_service"].get_remote_controller.return_value = mock_controller
        mock_keybindings_class = test_context.Mock()
        mock_keybindings_instance = test_context.Mock()
    ...
    def test_get_current_cell_basic(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch(
            "blind_x.table_navigator.focus_manager.get_manager",
            return_value=mock_focus_manager,
        )
        test_context.patch("blind_x.table_navigator.AXObject.get_parent", return_value=None)

        def mock_is_cell_or_header(obj):
            return obj == mock_cell

        test_context.patch(
            "blind_x.table_navigator.AXUtilities.is_table_cell_or_header",
            new=mock_is_cell_or_header,
        )
        test_context.patch("blind_x.table_navigator.AXUtilities.find_ancestor", return_value=None)
        test_context.patch("blind_x.table_navigator.debug.print_tokens", return_value=None)
        mock_controller = test_context.Mock()
        essential_modules["blind_x.dbus_service"].get_remote_controller.return_value = mock_controller
        mock_keybindings_class = test_context.Mock()
        mock_keybindings_instance = test_context.Mock()
    ...
    def test_get_current_cell_nested(self, test_context: OrcaTestContext) -> None:
        ...
        essential_modules = self._setup_dependencies(test_context)
        mock_focus_manager = test_context.Mock()
        essential_modules["blind_x.focus_manager"].get_manager.return_value = mock_focus_manager
        ...
        test_context.patch(
            "blind_x.table_navigator.AXObject.get_parent",
    ...
    def test_get_cell_coordinates_basic(self, test_context: OrcaTestContext) -> None:
        ...
        essential_modules = self._setup_dependencies(test_context)
        test_context.patch(
            "blind_x.table_navigator.AXTable.get_cell_coordinates",
            return_value=(2, 3),
        )
        mock_controller = test_context.Mock()
        essential_modules["blind_x.dbus_service"].get_remote_controller.return_value = mock_controller
    ...
    def test_get_cell_coordinates_with_previous(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.table_navigator.AXUtilities.get_table", return_value=mock_table)
        test_context.patch("blind_x.table_navigator.AXTable.get_cell_at", return_value=mock_cell)
        mock_controller = test_context.Mock()
        essential_modules["blind_x.dbus_service"].get_remote_controller.return_value = mock_controller
        mock_keybindings_class = test_context.Mock()
        mock_keybindings_instance = test_context.Mock()
    ...
    def test_move_direction_scenarios(
        ...
        pres_manager.present_message.assert_called_with(
            essential_modules["blind_x.messages"].TABLE_NOT_IN_A,
        )
        assert navigator._last_input_event == mock_event

        mock_cell = test_context.Mock(spec=Atspi.Accessible)
        test_context.patch(f"blind_x.table_navigator.AXTable.{boundary_check}", return_value=True)
        navigator._get_current_cell = test_context.Mock(return_value=mock_cell)
        pres_manager.present_message.reset_mock()
        result = move_method(mock_script, mock_event)
        assert result is True
        pres_manager.present_message.assert_called_with(
            getattr(essential_modules["blind_x.messages"], boundary_message),
        )

        mock_current_cell = test_context.Mock(spec=Atspi.Accessible)
        mock_next_cell = test_context.Mock(spec=Atspi.Accessible)
        test_context.patch(f"blind_x.table_navigator.AXTable.{boundary_check}", return_value=False)
        test_context.patch(
            f"blind_x.table_navigator.AXTable.{get_next_cell_method}",
    ...
    def test_move_left_successful(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.table_navigator.AXTable.is_start_of_row", return_value=False)
        test_context.patch(
            "blind_x.table_navigator.AXTable.get_cell_on_left",
    ...
    def test_move_right_successful(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.table_navigator.AXTable.is_end_of_row", return_value=False)
        test_context.patch(
            "blind_x.table_navigator.AXTable.get_cell_on_right",
    ...
    def test_move_up_at_top_of_column(self, test_context: OrcaTestContext) -> None:
        ...
        mock_cell = test_context.Mock(spec=Atspi.Accessible)
        test_context.patch("blind_x.table_navigator.AXTable.is_top_of_column", return_value=True)
        from blind_x.table_navigator import TableNavigator
        ...
        assert result is True
        pres_manager.present_message.assert_called_once_with(
            essential_modules["blind_x.messages"].TABLE_COLUMN_TOP,
        )
    ...
    def test_move_up_successful(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.table_navigator.AXTable.is_top_of_column", return_value=False)
        test_context.patch(
            "blind_x.table_navigator.AXTable.get_cell_above",
    ...
    def test_move_down_successful(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch(
            "blind_x.table_navigator.AXTable.is_bottom_of_column",
            return_value=False,
        )
        test_context.patch(
            "blind_x.table_navigator.AXTable.get_cell_below",
    ...
    def test_move_to_first_cell_successful(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.table_navigator.AXUtilities.get_table", return_value=mock_table)
        test_context.patch(
            "blind_x.table_navigator.AXTable.get_first_cell",
    ...
    def test_present_cell_successful(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.table_navigator.AXObject.grab_focus", return_value=None)
        test_context.patch(
            "blind_x.table_navigator.AXUtilities.get_descendant_supporting_text", return_value=None
        )
        test_context.patch("blind_x.table_navigator.AXObject.supports_text", return_value=False)
        test_context.patch("blind_x.table_navigator.AXTable.get_cell_spans", return_value=(1, 1))
        mock_focus_manager = test_context.Mock()
        essential_modules["blind_x.focus_manager"].get_manager.return_value = mock_focus_manager
        mock_controller = test_context.Mock()
        essential_modules["blind_x.dbus_service"].get_remote_controller.return_value = mock_controller
        ...
        test_context.patch("blind_x.table_navigator.AXUtilities.is_gui_cell", return_value=False)
    ...
    def test_get_navigator(self, test_context: OrcaTestContext) -> None:
        ...
        from blind_x import table_navigator
    ...
    def test_get_current_cell_needs_ancestor_search(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch(
            "blind_x.table_navigator.AXUtilities.find_ancestor",
    ...
    def test_move_left_skip_blank_cells(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch(
            "blind_x.table_navigator.AXTable.is_start_of_row",
        ...
        test_context.patch("blind_x.table_navigator.debug.print_tokens", return_value=None)
        test_context.patch("blind_x.table_navigator.TableNavigator._present_cell", return_value=None)
        mock_controller = test_context.Mock()
        essential_modules["blind_x.dbus_service"].get_remote_controller.return_value = mock_controller
        mock_keybindings_class = test_context.Mock()
        mock_keybindings_instance = test_context.Mock()
    ...
    def test_move_right_at_end_of_row(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.table_navigator.AXTable.get_cell_on_right", return_value=None)
        test_context.patch("blind_x.table_navigator.AXTable.is_end_of_row", return_value=True)
        test_context.patch("blind_x.table_navigator.debug.print_tokens", return_value=None)
        test_context.patch("blind_x.table_navigator.TableNavigator._present_cell", return_value=None)
        mock_focus_manager = test_context.Mock()
    ...
    def test_move_to_last_cell_successful(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.table_navigator.AXUtilities.get_table", return_value=mock_table)
        test_context.patch(
            "blind_x.table_navigator.AXTable.get_last_cell",
            return_value=mock_last_cell,
        )
        test_context.patch("blind_x.table_navigator.AXTable.get_row_count", return_value=5)
        test_context.patch("blind_x.table_navigator.AXTable.get_column_count", return_value=3)
        from blind_x.table_navigator import TableNavigator
    ...
    def test_move_to_top_of_column_successful(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.table_navigator.AXTable.is_top_of_column", return_value=False)
        test_context.patch(
            "blind_x.table_navigator.AXTable.get_top_of_column",
    ...
    def test_move_to_bottom_of_column_already_at_bottom(
        ...
        assert result is True
        pres_manager.present_message.assert_called_once_with(
            essential_modules["blind_x.messages"].TABLE_COLUMN_BOTTOM,
        )
    ...
    def test_move_to_bottom_of_column_successful(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch(
            "blind_x.table_navigator.AXTable.get_bottom_of_column",
    ...
    def test_move_to_beginning_of_row_already_at_start(self, test_context: OrcaTestContext) -> None:
        ...
        mock_cell = test_context.Mock(spec=Atspi.Accessible)
        test_context.patch("blind_x.table_navigator.AXTable.is_start_of_row", return_value=True)
        from blind_x.table_navigator import TableNavigator
        ...
        assert result is True
        pres_manager.present_message.assert_called_once_with(
            essential_modules["blind_x.messages"].TABLE_ROW_BEGINNING,
        )
    ...
    def test_move_to_beginning_of_row_successful(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.table_navigator.AXTable.is_start_of_row", return_value=False)
        test_context.patch(
            "blind_x.table_navigator.AXTable.get_start_of_row",
    ...
    def test_move_to_end_of_row_already_at_end(self, test_context: OrcaTestContext) -> None:
        ...
        assert result is True
        pres_manager.present_message.assert_called_once_with(
            essential_modules["blind_x.messages"].TABLE_ROW_END,
        )
    ...
    def test_move_to_end_of_row_successful(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.table_navigator.AXTable.is_end_of_row", return_value=False)
        test_context.patch(
            "blind_x.table_navigator.AXTable.get_end_of_row",
    ...
    def test_set_dynamic_column_headers_row_successful(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch(
            "blind_x.table_navigator.AXUtilities.set_dynamic_column_headers_row",
            return_value=None,
        )
        essential_modules["blind_x.messages"].DYNAMIC_COLUMN_HEADER_SET = "Column header row set to %d"
        from blind_x.table_navigator import TableNavigator
    ...
    def test_clear_dynamic_column_headers_row_successful(
        ...
        test_context.patch("blind_x.table_navigator.AXUtilities.get_table", return_value=mock_table)
        test_context.patch(
            "blind_x.table_navigator.AXUtilities.clear_dynamic_column_headers_row",
            return_value=None,
        )
        from blind_x.table_navigator import TableNavigator
        ...
        pres_manager.present_message.assert_called_once_with(
            essential_modules["blind_x.messages"].DYNAMIC_COLUMN_HEADER_CLEARED,
        )
    ...
    def test_set_dynamic_row_headers_column_successful(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch(
            "blind_x.table_navigator.AXUtilities.set_dynamic_row_headers_column",
            return_value=None,
        )
        essential_modules["blind_x.messages"].DYNAMIC_ROW_HEADER_SET = "Row header column set to %s"
        test_context.patch(
            "blind_x.table_navigator.AXUtilities.get_column_label",
            create=True,
            return_value="B",
        )
        from blind_x.table_navigator import TableNavigator
    ...
    def test_clear_dynamic_row_headers_column_successful(
        ...
        test_context.patch("blind_x.table_navigator.AXUtilities.get_table", return_value=mock_table)
        test_context.patch(
            "blind_x.table_navigator.AXUtilities.clear_dynamic_row_headers_column",
            return_value=None,
        )
        from blind_x.table_navigator import TableNavigator
        ...
        pres_manager.interrupt_presentation.assert_called_once()
        pres_manager.present_message.assert_called_once_with(
            essential_modules["blind_x.messages"].DYNAMIC_ROW_HEADER_CLEARED,
        )
    ...
    def test_get_cell_coordinates_with_different_previous_cell(
        ...
        test_context.patch("blind_x.table_navigator.AXUtilities.get_table", return_value=mock_table)
        test_context.patch(
            "blind_x.table_navigator.AXTable.get_cell_at",
    ...
    def test_present_cell_with_settings_enabled(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.table_navigator.AXObject.supports_text", return_value=False)
        test_context.patch("blind_x.table_navigator.AXTable.get_cell_spans", return_value=(2, 3))
        mock_focus_manager = test_context.Mock()
        essential_modules["blind_x.focus_manager"].get_manager.return_value = mock_focus_manager
        essential_modules["blind_x.messages"].TABLE_CELL_COORDINATES = "Row %(row)s, column %(column)s"
        essential_modules["blind_x.messages"].cell_span.return_value = "spans 2 rows and 3 columns"
        mock_controller = test_context.Mock()
        essential_modules["blind_x.dbus_service"].get_remote_controller.return_value = mock_controller
        mock_keybindings_class = test_context.Mock()
        mock_keybindings_instance = test_context.Mock()
    ...
    def test_present_cell_with_text_descendant_and_gui_cell(
        ...
        test_context.patch("blind_x.table_navigator.AXObject.grab_focus", return_value=None)
        test_context.patch(
            "blind_x.table_navigator.AXUtilities.get_descendant_supporting_text",
            return_value=mock_text_obj,
        )
        test_context.patch(
            "blind_x.table_navigator.AXObject.supports_text",
        ...
        test_context.patch("blind_x.table_navigator.AXUtilities.is_gui_cell", return_value=True)
    ...
    def test_get_skip_blank_cells(self, test_context: OrcaTestContext) -> None:
        ...
        from blind_x import gsettings_registry
        from blind_x.table_navigator import get_navigator
    ...
    def test_present_cell_emits_region_changed(self, test_context: OrcaTestContext) -> None:
        ...
        from blind_x import focus_manager, gsettings_registry

        essential_modules["blind_x.focus_manager"].TABLE_NAVIGATOR = focus_manager.TABLE_NAVIGATOR
        registry = gsettings_registry.get_registry()
        registry.set_runtime_value("speech", "announce-cell-coordinates", False)
        registry.set_runtime_value("speech", "announce-cell-span", False)
