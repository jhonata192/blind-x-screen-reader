...
class TestEventManager:
    ...
    def _setup_ignore_event_ax_utilities_mocks(self, test_context, mock_time=5000.0, is_text=True):
        ...
        test_context.patch("blind_x.event_manager.AXUtilities.is_text", return_value=is_text)
        test_context.patch("blind_x.event_manager.AXUtilities.is_window", return_value=False)
        test_context.patch("blind_x.event_manager.AXUtilities.is_frame", return_value=False)
        test_context.patch("blind_x.event_manager.AXUtilities.is_notification", return_value=False)
        test_context.patch("blind_x.event_manager.AXUtilities.is_alert", return_value=False)
        test_context.patch("blind_x.event_manager.AXUtilities.is_selected", return_value=False)
        test_context.patch("blind_x.event_manager.AXUtilities.is_focused", return_value=False)
        test_context.patch("blind_x.event_manager.AXUtilities.is_section", return_value=False)
    ...
    def _setup_dependencies(self, test_context: OrcaTestContext) -> dict[str, MagicMock]:
        ...
        additional_modules = [
            ...
            "blind_x.blind_x_modifier_manager",
            "blind_x.ax_utilities_debugging",
        ...
        input_event_manager_mock = essential_modules["blind_x.input_event_manager"]
        ...
        ax_utils_debugging_mock = essential_modules["blind_x.ax_utilities_debugging"]
    ...
    def test_init(self, test_context: OrcaTestContext) -> None:
        ...
        from blind_x.event_manager import EventManager
    ...
    def test_activation_operations(
        ...
        test_context.patch(
            "blind_x.event_manager.input_event_manager.get_manager",
    ...
    def test_get_priority(self, test_context, case: dict) -> None:
        ...
        ax_utilities = essential_modules["blind_x.ax_utilities"]
    ...
    def test_get_priority_announcement_levels(self, test_context: OrcaTestContext) -> None:
        ...
        self._setup_dependencies(test_context)
        from blind_x.event_manager import EventManager, EventPriority
    ...
    def test_ignore_frame_events(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.event_manager.AXUtilities.is_window", return_value=False)
        test_context.patch("blind_x.event_manager.AXUtilities.is_frame", return_value=True)
        test_context.patch("blind_x.event_manager.AXUtilities.get_application", return_value=mock_app)
        test_context.patch("blind_x.event_manager.AXUtilities.is_mutter_x11_frames", return_value=True)
        ...
        test_context.patch("blind_x.event_manager.AXUtilities.is_frame", return_value=True)
        test_context.patch(
            "blind_x.event_manager.AXUtilities.get_application",
            return_value=regular_app,
        )
        test_context.patch(
            "blind_x.event_manager.AXUtilities.is_mutter_x11_frames", return_value=False
        )
    ...
    def test_ignore_focused_source_events(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.event_manager.AXUtilities.is_window", return_value=False)
        test_context.patch("blind_x.event_manager.AXUtilities.is_frame", return_value=False)
        test_context.patch("blind_x.event_manager.AXUtilities.is_text", return_value=False)
        test_context.patch("blind_x.event_manager.AXUtilities.is_notification", return_value=False)
        test_context.patch("blind_x.event_manager.AXUtilities.is_alert", return_value=False)
        test_context.patch("blind_x.event_manager.AXUtilities.is_selected", return_value=False)
        test_context.patch("blind_x.event_manager.AXUtilities.is_focused", return_value=True)
        test_context.patch("blind_x.event_manager.AXUtilities.manages_descendants", return_value=False)
        assert manager._ignore(mock_event) is False
        ...
        test_context.patch("blind_x.event_manager.AXUtilities.is_selected", return_value=False)
        test_context.patch("blind_x.event_manager.AXUtilities.is_focused", return_value=True)
        test_context.patch("blind_x.event_manager.AXUtilities.manages_descendants", return_value=True)
        assert manager._ignore(mock_event) is False
    ...
    def test_ignore_live_region_events(self, test_context: OrcaTestContext) -> None:
        ...
        mock_event.detail2 = 100
        ax_utilities = essential_modules["blind_x.ax_utilities"]
        ax_object = essential_modules["blind_x.ax_object"]
        focus_mgr = essential_modules["focus_manager_instance"]
    ...
    def test_ignore_spam_filtering(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.event_manager.AXObject.get_attribute", return_value=None)
        test_context.patch("blind_x.event_manager.focus_manager.get_manager", return_value=focus_mgr)
        assert manager._ignore(mock_event) is False
    ...
    def test_get_priority_various_event_types(
        ...
        test_context.patch("blind_x.event_manager.AXUtilities.is_frame", return_value=case["is_frame"])
        test_context.patch(
            "blind_x.event_manager.AXUtilities.is_dialog_or_alert",
            return_value=case["is_dialog_or_alert"],
        )

        result = manager._get_priority(mock_event)
        assert result == case["expected_priority"]
        essential_modules["blind_x.debug"].print_tokens.assert_called()
    ...
    def test_ignore_event_conditions(
        ...
        test_context.patch("time.time", return_value=100.0)
        test_context.patch(
            "blind_x.event_manager.AXUtilities.is_window",
        ...
        test_context.patch("blind_x.event_manager.AXUtilities.is_selected", return_value=False)
        test_context.patch(
            "blind_x.event_manager.AXUtilities.is_focused",
            side_effect=lambda obj: (
        ...
        test_context.patch("blind_x.event_manager.AXUtilities.is_section", return_value=False)
        test_context.patch(
            "blind_x.event_manager.AXUtilities.manages_descendants",
            side_effect=lambda obj: (
    ...
    def test_ignore_live_region_text_insertion(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.event_manager.AXUtilities.is_selected", return_value=False)
        test_context.patch("blind_x.event_manager.AXUtilities.is_focused", return_value=False)
        test_context.patch("blind_x.event_manager.AXUtilities.is_section", return_value=True)
        test_context.patch("blind_x.event_manager.AXUtilities.manages_descendants", return_value=False)
        test_context.patch("blind_x.event_manager.AXUtilities.get_application", return_value=mock_app)
        test_context.patch(
            "blind_x.event_manager.AXUtilities.is_mutter_x11_frames", return_value=False
        ...
        test_context.patch(
            "blind_x.event_manager.AXObject.get_attribute",
    ...
    def test_is_obsoleted_by_sibling_events(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch(
            "blind_x.event_manager.AXObject.get_parent",
    ...
    def test_ignore_selected_source_not_ignored(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.event_manager.AXUtilities.is_alert", return_value=False)
        test_context.patch(
            "blind_x.event_manager.AXUtilities.is_selected",
    ...
    def test_on_no_focus(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.event_manager.focus_manager.get_manager", new=mock_get_focus_mgr)
        mock_get_script_mgr = test_context.Mock()
        test_context.patch("blind_x.event_manager.script_manager.get_manager", new=mock_get_script_mgr)
        ...
        test_context.patch("blind_x.event_manager.braille_presenter", new=mock_braille_presenter)
    ...
    def test_get_script_for_event_mouse(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.event_manager.script_manager.get_manager", new=mock_get_script_mgr)
        test_context.patch(
            "blind_x.event_manager.input_event.MouseButtonEvent",
    ...
    def test_process_object_event_dead_source(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch_object(manager, "_is_obsoleted_by", new=mock_is_obsoleted)
        test_context.patch("blind_x.event_manager.AXObject.is_dead", return_value=True)
        test_context.patch("blind_x.event_manager.AXUtilities.is_defunct", return_value=False)
        mock_get_focus_mgr = test_context.Mock()
        test_context.patch("blind_x.event_manager.focus_manager.get_manager", new=mock_get_focus_mgr)
        mock_get_script_mgr = test_context.Mock()
        test_context.patch("blind_x.event_manager.script_manager.get_manager", new=mock_get_script_mgr)
    ...
    def test_get_manager(self, test_context: OrcaTestContext) -> None:
        ...
        self._setup_dependencies(test_context)
        from blind_x import event_manager
    ...
    def test_enqueue_with_comprehensive_gidle_management(
        ...
        test_context.patch("blind_x.event_manager.GLib.idle_add", new=mock_idle_add)
