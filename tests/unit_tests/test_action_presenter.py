...
class TestActionPresenter:
    ...
    def _setup_dependencies(self, test_context: OrcaTestContext) -> dict[str, MagicMock]:
        ...
        essential_modules["blind_x.cmdnames"].SHOW_ACTIONS_LIST = SHOW_ACTIONS_LIST_CMD
        ...
        essential_modules["blind_x.focus_manager"].get_manager = test_context.Mock(
        ...
        essential_modules["blind_x.input_event"].InputEvent = test_context.mocker.MagicMock

        essential_modules["blind_x.messages"].LOCATION_NOT_FOUND_FULL = LOCATION_NOT_FOUND_FULL_MSG
        essential_modules["blind_x.messages"].LOCATION_NOT_FOUND_BRIEF = LOCATION_NOT_FOUND_BRIEF_MSG
        essential_modules["blind_x.messages"].NO_ACTIONS_FOUND_ON = "No actions found on %s"
        essential_modules["blind_x.guilabels"].ACTIONS_LIST = "Actions List"
    ...
    def test_show_actions_list(  # pylint: disable=too-many-arguments,too-many-positional-arguments,too-many-locals,too-many-statements
        ...
        if test_scenario != "no_object":
            ...
            test_context.patch_module("blind_x.ax_action", ax_action_module_mock)

            ax_object_module_mock = test_context.Mock()
            ax_object_module_mock.AXObject = ax_object_mock
            test_context.patch_module("blind_x.ax_object", ax_object_module_mock)
            ax_utilities_module_mock = test_context.Mock()
            ax_utilities_module_mock.AXUtilities = ax_utilities_mock
            test_context.patch_module("blind_x.ax_utilities", ax_utilities_module_mock)

        from blind_x.action_presenter import ActionPresenter

        presenter = ActionPresenter()
        mock_script = essential_modules["blind_x.script_manager"].get_manager().get_script()
        mock_event = test_context.Mock()
        focus_manager = essential_modules["blind_x.focus_manager"].get_manager()
        ...
        if expects_gui_creation:
            ...
            mock_action_list_class = test_context.patch(
                "blind_x.action_presenter.ActionList",
    ...
    def test_perform_action(
        ...
        mock_idle_add = test_context.patch("blind_x.action_presenter.GLib.idle_add")
        mock_debug_tokens = test_context.patch("blind_x.action_presenter.debug.print_tokens")
        mock_do_action = test_context.patch("blind_x.action_presenter.AXUtilities.do_named_action")
        ...
        if gui_state is None:
            ...
            debug_call = essential_modules["blind_x.debug"].print_message.call_args[0][1]
    ...
    def test_restore_focus_sets_script_and_focus(self, test_context: OrcaTestContext) -> None:
        ...
        test_context.patch("blind_x.action_presenter.AXUtilities", new=ax_utilities_mock)

        script_manager_instance = essential_modules["blind_x.script_manager"].get_manager()
        script_manager_instance.get_script.return_value = mock_script

        focus_manager_instance = essential_modules["blind_x.focus_manager"].get_manager()
