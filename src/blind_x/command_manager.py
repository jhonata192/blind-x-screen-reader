...
class KeybindingsPreferencesGrid(preferences_grid_base.PreferencesGridBase):
    ...
    def revert_changes(self) -> None:
        """Revert keyboard layout and blind_x modifier to their original values."""
    ...
    def _process_key_captured(self, event: Gdk.EventKey) -> bool:
        ...
        if not self._captured_key[0]:
            # Preserve Blind-X modifier if it was already captured.
        ...
        # Preserve Blind-X modifier from previous key if present before comparing
    ...
    def refresh(self) -> None:
        """Refresh the keyboard layout and blind_x modifier displays."""
...
@gsettings_registry.get_registry().gsettings_enum(
    "org.gnome.BlindX.KeyboardLayout",
...
class CommandManager:  # pylint: disable=too-many-instance-attributes
    ...
    @gsettings_registry.get_registry().gsetting(
        ...
        summary="Keys used as the Blind-X modifier for the desktop layout",
    )
    @dbus_service.getter
    ...
    @gsettings_registry.get_registry().gsetting(
        ...
        summary="Keys used as the Blind-X modifier for the laptop layout",
    )
    @dbus_service.getter
    ...
    def check_keyboard_settings(self) -> None:
        ...
        msg = f"COMMAND MANAGER: Modifier keys changing to {mod_mgr.get_blind_x_modifier_keys()}"
        debug.print_message(debug.LEVEL_INFO, msg, True)
        mod_mgr.refresh_blind_x_modifiers("Keyboard settings changed.")
        ...
        def update_grabs() -> bool:
            for cmd in self._keyboard_commands.values():
                ...
                if self._numlock_on and kb.has_grabs():
                    ...
                    kb.add_grabs(blind_x_modifiers)
            return False
        ...
        for key, new_kb in new_bindings.items():
            if key not in transferred_keys and not new_kb.has_grabs():
                if self._is_desktop and self._numlock_on and key[0].startswith("KP_"):
                    continue
                new_kb.add_grabs(blind_x_modifiers)
                if new_kb.has_grabs():
                    added.append(key)
