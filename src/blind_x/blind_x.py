...
# Copyright 2010-2011 The Blind-X Team
...
"""The main module for the Blind-X screen reader."""
...
def load_user_settings(script=None, skip_reload_message=False, is_reload=True):
    ...
    debug.print_message(debug.LEVEL_INFO, "BLIND-X: Loading User Settings", True)
...
def shutdown(_event=None, _signum=None):
    """Exits Blind-X. Returns True if shutdown ran to completion."""

    if hasattr(shutdown, "in_progress"):
        msg = "BLIND-X: Shutdown already in progress"
    ...
    debug.print_message(debug.LEVEL_INFO, "BLIND-X: Shutting down", True)
    ...
    signal.alarm(0)
    debug.print_message(debug.LEVEL_INFO, "BLIND-X: Quitting Atspi main event loop", True)
    Atspi.event_quit()  # pylint: disable=no-value-for-parameter
    debug.print_message(debug.LEVEL_INFO, "BLIND-X: Shutdown complete", True)
    debug.shutdown()
    return True
...
def _setup_signal_handlers():
    ...
    def _reload_on_signal(signum, frame):
        ...
        tokens = [f"BLIND-X: Reloading due to signal={signum} {signal_string}", frame]
    ...
    def _shutdown_on_signal(signum, frame):
        signal_string = f"({signal.strsignal(signum)})"
        msg = f"BLIND-X: Shutting down and exiting due to signal={signum} {signal_string}"
        tokens = [msg, frame]
        debug.print_tokens(debug.LEVEL_INFO, tokens, True)
        shutdown()
    ...
    def _show_preferences_on_signal(signum, frame):
        signal_string = f"({signal.strsignal(signum)})"
        tokens = [f"BLIND-X: Showing preferences due to signal={signum} {signal_string}", frame]
        debug.print_tokens(debug.LEVEL_INFO, tokens, True)
    ...
    def _glib_shutdown_handler():
        msg = "BLIND-X: GLib handler received shutdown signal"
        debug.print_message(debug.LEVEL_INFO, msg, True)
        shutdown()
        return GLib.SOURCE_REMOVE

    def _glib_reload_handler():
        msg = "BLIND-X: GLib handler received reload signal"
        debug.print_message(debug.LEVEL_INFO, msg, True)
        systemd.get_manager().notify_reloading()
        load_user_settings()
        systemd.get_manager().notify_ready()
...
def _ensure_accessibility_enabled() -> int | None:
    ...
    try:
        ...
        msg = f"BLIND-X: Accessibility enabled: {enabled}"
        debug.print_message(debug.LEVEL_INFO, msg, True)
        if not enabled:
            msg = "BLIND-X: Enabling accessibility."
            debug.print_message(debug.LEVEL_INFO, msg, True)
            proxy.Set("org.a11y.Status", "IsEnabled", GLib.Variant("b", True))
    except GLib.GError as error:
        msg = f"BLIND-X: Could not connect to D-Bus session bus: {error}"
        debug.print_message(debug.LEVEL_SEVERE, msg, True)
...
def _activate_services():
    """Activates Blind-X services and attempts to find the focused object."""
...
def main():
    """The main entry point for Blind-X."""
    ...
    msg = f"BLIND-X: Running under systemd: {is_systemd_managed}"
    debug.print_message(debug.LEVEL_INFO, msg, True)
    ...
    try:
        debug.print_message(debug.LEVEL_INFO, "BLIND-X: Starting Atspi main event loop", True)
        Atspi.event_main()  # pylint: disable=no-value-for-parameter
    except GLib.Error as error:
        msg = f"BLIND-X: Exception starting ATSPI registry: {error}"
        debug.print_message(debug.LEVEL_SEVERE, msg, True)
        os.kill(os.getpid(), signal.SIGKILL)
    return 0
