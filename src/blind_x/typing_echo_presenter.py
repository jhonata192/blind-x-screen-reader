...
@gsettings_registry.get_registry().gsettings_schema(
    "org.gnome.BlindX.TypingEcho", name="typing-echo"
)
class TypingEchoPresenter(Extension):
    ...
    def _should_echo_blind_x_modifier(self, event: input_event.KeyboardEvent) -> bool:
        """Returns whether an Blind-X modifier event should be echoed."""

        click_count = event.get_click_count()
        if click_count == 2:
            msg = "TYPING ECHO PRESENTER: Echoing Blind-X modifier double-click."
            debug.print_message(debug.LEVEL_INFO, msg, True)
            return True
        ...
        msg = f"TYPING ECHO PRESENTER: Echoing modifier Blind-X modifier event: {result}."
        debug.print_message(debug.LEVEL_INFO, msg, True)
        return result
    ...
    def should_echo_keyboard_event(self, event: input_event.KeyboardEvent) -> bool:
        ...
        if event.is_blind_x_modifier():
            return self._should_echo_blind_x_modifier(event)

        # Treat all command modifiers the same and suppress echo.
        if event.is_alt_control_or_blind_x_modified() or self.is_character_echoable(event):
            reason = (
                "modifier" if event.is_alt_control_or_blind_x_modified() else "character echoable"
            )
    ...
    def is_character_echoable(self, event: input_event.KeyboardEvent) -> bool:
        ...
        if event.is_alt_control_or_blind_x_modified():
