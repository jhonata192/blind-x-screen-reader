...
"""Messages which Blind-X presents in speech and/or braille. These
...
# Translators: Blind-X has a command which toggles all (other) Blind-X commands so that
# the associated keystroke can by consumed by the native application. For example,
# if there were an Blind-X command bound to Alt+Down, normally pressing Alt+Down
# would cause the Blind-X command to be used. This would mean Alt+Down could not be
# used in editors to move the current line of text down. By temporarily disabling
# Blind-X commands, Alt+Down would be ignored by Blind-X and work as expected in the
# editor. This string is what Blind-X presents to the user when Blind-X's commands are
# being toggled off.
BYPASS_MODE_ENABLED = _("Blind-X command keys off.")
...
BYPASS_MODE_DISABLED = _("Blind-X command keys on.")
...
# non-verbose output presented in response to the use of an Blind-X command which
...
# output presented in response to the use of an Blind-X command which makes it
...
# Blind-X user wants. As such, Blind-X offers the user the ability to toggle between
# the application controlling the caret and Blind-X controlling it. This message
# is presented to indicate that the application's native caret navigation is
# active / not being overridden by Blind-X.
CARET_CONTROL_APP = _("The application is controlling the caret.")

# Translators: Gecko native caret navigation is where Firefox (or Thunderbird)
# itself controls how the arrow keys move the caret around HTML content. It's
# often broken, so Blind-X needs to provide its own support. As such, Blind-X offers
...
# Translators: This string appears when using 'Blind-X -h' at the command line.
# It serves as a sort of title and is followed by a detailed list of Blind-X's
# optional command-line arguments.
CLI_OPTIONAL_ARGUMENTS = _("Optional arguments")

# Translators: This string appears when using 'Blind-X -h' at the command line.
# It is followed by a brief list of Blind-X's optional command-line arguments.
CLI_USAGE = _("Usage: ")

# Translators: This message is displayed when the user starts Blind-X from the
...
# which prints the version of Blind-X. E.g. '1.23.4'.
CLI_VERSION = _("Version of this application")

# Translators: This is the description of command line option '-r, --replace'
# which tells Blind-X to replace any existing Blind-X process that might be running.
...
# causes debugging output for Blind-X to be sent to a file. The YYYY-MM-DD-HH:MM:SS
...
# that will place the user in Blind-X's GUI preferences dialog.
CLI_GUI_SETUP = _("Set up user preferences (GUI version)")

# Translators: This text is the description displayed when Blind-X is launched
# from the command line and the help text is displayed.
CLI_EPILOG = _("Report bugs on https://github.com/jhonata192/blind-x-screen-reader/issues.")

# Translators: Blind-X has a command to present the contents of the clipboard without
# the user having to switch to a clipboard manager. This message is spoken by Blind-X
...
# Translators: Blind-X normal speaks the text which was just deleted from a
# document via command. Depending on the circumstances, that might be a
# large string. Therefore, if the text which has just been deleted from a
# document matches the clipboard contents, Blind-X will indicate that fact
...
# Translators: Blind-X normal speaks the text which was just inserted into a
# document via command. Depending on the circumstances, that might be a
...
# to have this typing status announced by Blind-X; others find that announcement
# unpleasant. Therefore, it is a setting in Blind-X. This string to be translated
...
# Translators: In chat applications, Blind-X automatically presents incoming
...
# messages in speech and braille. If a user is in multiple conversations or
# channels at the same time, it can be confusing to know what room or channel
# a given message came from just from hearing/reading it. For this reason, Blind-X
...
# Translators: Blind-X has a command to review previous chat room messages in
# speech and braille. Some users prefer to have this message history combined
# (e.g. the last ten messages which came in, no matter what room they came
# from). Other users prefer to have specific room history (e.g. the last ten
# messages from #a11y). Therefore, this is a setting in Blind-X. This string to be
...
# Translators: Blind-X has a command to report CPU and memory usage. This message
# is presented to the user when they use this command but Blind-X was unable to
# retrieve this information.
CPU_AND_MEMORY_USAGE_UNKNOWN = _("CPU and memory usage unknown")

# Translators: Blind-X has a command to report CPU and memory usage levels. This
# message presents the levels to the user.
CPU_AND_MEMORY_USAGE_LEVELS = _("CPU: %d%%. Memory: %d%%")

# Translators: Blind-X has a command for advanced users and developers to clear
...
# Translators: The "default" button in a dialog box is the button that gets
# activated when Enter is pressed anywhere within that dialog box. Blind-X has
# a command to present the default button. This is the message Blind-X will
...
# document, Blind-X will speak information about what they have selected or
...
# Translators: Blind-X allows you to dynamically define which row of a spreadsheet
...
# Translators: Blind-X allows you to dynamically define which column of a
...
# Translators: This is a spoken and/or brailled message letting the user know
# that Blind-X has recorded a new key combination (e.g. Alt+Ctrl+g) as a result of
# their input. The string substituted in is the new key combination.
KB_CAPTURED = _("Key captured: %s. Press enter to confirm.")

# Translators: This is a spoken and/or brailled message letting the user know
# that Blind-X has assigned a new key combination (e.g. Alt+Ctrl+g) as a result of
# their input. The string substituted in is the new key combination.
KB_CAPTURED_CONFIRMATION = _("The new key is: %s")

# Translators: This is a spoken and/or brailled message letting the user know
# Blind-X is about to delete an existing key combination (e.g. Alt+Ctrl+g) as a
# result of their input.
KB_DELETED = _("Key binding deleted. Press enter to confirm.")

# Translators: This is a spoken and/or brailled message letting the user know
# Blind-X has deleted an existing key combination (e.g. Alt+Ctrl+g) as a result of
# their input.
...
# Translators: Blind-X has an "echo" setting which allows the user to configure
...
# Translators: Blind-X announces when a widget has an associated error, such as
...
# goes away as a consequence of the user fixing the error, Blind-X will present
...
# Translators: Blind-X has a "Learn Mode" that will allow the user to type any key
# on the keyboard and hear what the effects of that key would be.  The effects
# might be what Blind-X would do if it had a handler for the particular key
# combination, or they might just be to echo the name of the key if Blind-X doesn't
...
# idle ("polite"), and when there is a change ("assertive"). Blind-X has several
# features to facilitate accessing live regions. This message is presented to
# inform the user that Blind-X's live region's "politeness" level has changed to
...
# idle ("polite"), and when there is a change ("assertive"). Blind-X has several
# features to facilitate accessing live regions. This message is presented to
# inform the user that Blind-X's live region's "politeness" level for all live
...
# idle ("polite"), and when there is a change ("assertive"). Blind-X has several
# features to facilitate accessing live regions. This message is presented to
# inform the user that Blind-X's live region's "politeness" level has changed for
...
# Translators: A live region is an area of a web page that is periodically
# updated, e.g. stock ticker. https://w3c.github.io/aria/#dfn-live-region
# Blind-X has several features to facilitate accessing live regions. This message
# is presented in response to a command that toggles whether or not Blind-X pays
...
# Translators: Blind-X has a command that allows the user to move the mouse pointer
# to the current object. This is a brief message which will be presented if for
# some reason Blind-X cannot identify/find the current location.
LOCATION_NOT_FOUND_BRIEF = C_("location", "Not found")

# Translators: Blind-X has a command that allows the user to move the mouse pointer
# to the current object. This is a detailed message which will be presented if
# for some reason Blind-X cannot identify/find the current location.
...
# Translators: Blind-X has a command to present the state of modifier keys. This message
...
# Translators: Blind-X tries to provide more compelling output of the spell check
...
# Translators: Blind-X has a number of commands that override the default
...
# if you are editing the text in the entry. Because Blind-X doesn't know
# what you want to do, it has two modes: In browse mode, Blind-X treats
# key presses as commands to read the content; in focus mode, Blind-X treats
# key presses as something that should be handled by the focused widget.
# This string is the message presented when Blind-X switches to browse mode.
...
# key presses as commands to read the content; in focus mode, Blind-X treats
# key presses as something that should be handled by the focused widget.
# This string is the message presented when Blind-X switches to focus mode.
MODE_FOCUS = _("Focus mode")

# Translators: (Please see the previous, detailed translator notes about
# Focus mode and Browse mode.) In order to minimize the amount of work Blind-X
# users need to do to switch between focus mode and browse mode, Blind-X attempts
# to automatically switch to the mode which is appropriate to the current
# web element. Sometimes, however, this automatic mode switching is not what
# the user wants. A good example being web apps which have their own keyboard
# navigation and use interaction model. As a result, Blind-X has a command which
# enables setting a "sticky" focus mode which disables all automatic toggling.
# This string is the message presented when Blind-X switches to sticky focus mode.
...
# This string is the message presented when Blind-X switches to sticky browse mode.
MODE_BROWSE_IS_STICKY = _("Browse mode is sticky.")

# Translators: Blind-X has two modes for reading web pages and similar content:
# focus mode and browse mode. Focus mode passes key presses to the application
# for handling. Browse mode intercepts keys for Blind-X commands and caret navigation.
...
# switches to layout mode via an Blind-X command.
...
# it is disabled, Blind-X will treat each object as if it were on a separate line,
# both for presentation and navigation. This string is presented when the user
# toggles layout mode off via an Blind-X command and switches to the aforementioned
# object-based presentation.
MODE_OBJECT = _("Object mode.")

# Translators: Blind-X supports desktop and laptop keyboard layouts. This message
...
# Translators: Blind-X has a feature to speak the item under the pointer. This feature,
# known as mouse review, can be enabled and disabled via command. The following is
# the message which Blind-X will present when mouse review is toggled off via command.
MOUSE_REVIEW_DISABLED = _("Mouse review disabled.")

# Translators: Blind-X has a feature to speak the item under the pointer. This feature,
# known as mouse review, can be enabled and disabled via command. The following is
# the message which Blind-X will present when mouse review is toggled on via command.
...
# Translators: Blind-X has a command which presents a menu with accessible actions
# that can be performed on the current object. This is the message that Blind-X
...
# Translators: Blind-X has a dedicated command to speak the currently-selected
# text. This message is what Blind-X will present if the user performs this
# command when no text is selected.
NO_SELECTED_TEXT = _("No selected text.")

# Translators: Blind-X has a dedicated command to speak detailed information
# about the currently-focused link. This message is what Blind-X will present
# if the user performs this command when not on a link.
NOT_ON_A_LINK = _("Not on a link.")

# Translators: Blind-X has commands to navigate among objects by object type.
# When the user moves to a focusable object, such as a button, Blind-X grabs
...
# updating the focus in response to a grab. Therefore Blind-X will present this
...
# Translators: Blind-X has a setting through which users can control how a number is
# spoken. The options are digits ("1 2 3") and words ("one hundred and twenty
# three"). There is an associated Blind-X command for quickly toggling between the
...
# Translators: This message is presented to the user when Blind-X's preferences
# have been reloaded.
SETTINGS_RELOADED = _("Screen reader settings reloaded.")

# Translators: Blind-X has a dedicated command to speak the currently-selected
# text. This message is spoken by Blind-X before speaking the text which is
...
# document via command. Depending on the circumstances, that might be a
# large string. Therefore, if the text which has just been deleted from a
# document matches the previously-selected contents, Blind-X will indicate that
...
# and then pressed Ctrl+Z to undo that deletion). In this instance, Blind-X
...
# Translators: Blind-X has a command which presents the size and position of the
...
# resume working. This string is the message Blind-X presents when sleep mode is
...
# Translators: Blind-X's verbosity levels control how much (or how little)
# Blind-X will speak when presenting objects as the user navigates within
...
# Translators: This message is presented to the user when Blind-X is launched.
START_ORCA = _("Screen reader on.")

# Translators: This message is presented to the user when Blind-X is quit.
STOP_ORCA = _("Screen reader off.")

# Translators: Blind-X has a command to present the contents of the status bar.
# This is a brief message which will be presented if Blind-X cannot find the
# status bar (e.g. because there isn't one).
STATUS_BAR_NOT_FOUND_BRIEF = C_("status bar", "Not found")

# Translators: Blind-X has a command to present the contents of the status bar.
# This is a detailed message which will be presented if Blind-X cannot find the
# status bar (e.g. because there isn't one).
STATUS_BAR_NOT_FOUND_FULL = _("Status bar not found")

# Translators: the Blind-X "Find" dialog allows a user to search for text in a
...
# is presented when the user disables the structural navigation feature of Blind-X.
...
# Translators: Blind-X has a command that allows the user to move to the next
# structural navigation object. In Blind-X, "structural navigation" refers to
...
# quickly moving through a container by jumping amongst objects of a given
# type, such as from link to link, or from heading to heading, or from form
# field to form field. In order for this functionality to work, Blind-X uses
# the AtspiCollection interface. If an object claims to not support that
# interface, Blind-X will present this message to the user to indicate that
...
# Translators: Blind-X has commands for navigating within a table, e.g. to the
...
# Translators: Blind-X has a modal math navigation mode for exploring math
...
# Translators: when the user selects (highlights) text in a document, Blind-X lets
# them know.
TEXT_SELECTED = C_("text", "selected")

# Translators: when the user unselects (un-highlights) text in a document, Blind-X
# lets them know.
TEXT_UNSELECTED = C_("text", "unselected")

# Translators: Blind-X has a feature to speak the time when the user presses a
...
# Translators: This message presents the Blind-X version number.
...
def items_found(count):
    ...
    # Translators: Blind-X has several commands that search for, and present a list
...
def list_item_count(count):
    ...
    if count == -1:
        ...
        # number. Normally Blind-X announces "list with n items" when the count is
...
def feed_article_count(count):
    ...
    if count == -1:
        ...
        # Normally Blind-X announces "feed with n articles" when the count is known.
...
def repeated_char_count(repeated_char, count):
    ...
    # Translators: Blind-X will tell you how many characters are repeated on a line
...
def shortcuts_found_blind_x(count):
    """Returns a string description of the number of Blind-X shortcuts found."""

    # Translators: This message is presented when the user is in a list of
    # shortcuts associated with Blind-X commands. It appears as the title of the
...
def table_size(n_rows, n_columns):
    ...
    if n_rows == -1:
        if n_columns == -1:
            ...
            # Blind-X announces the table dimensions (e.g. "table with 100 rows
