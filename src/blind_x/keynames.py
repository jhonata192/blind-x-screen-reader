...
# Translators: this string refers to the "Blind-X modifier" (typically CapsLock on a laptop
# and KP_Insert on a desktop). Blind-X takes over this key in order to provide additional
# commands without conflicting with the application being used.
_keynames["Blind-X"] = C_("keyboard", "Blind-X")
...
def get_click_count_string(count: int) -> str:
    ...
    if count == 2:
        # Translators: Blind-X keybindings support double and triple "clicks" or
        # key presses, similar to using a mouse.
        return _("double click")
    if count == 3:
        # Translators: Blind-X keybindings support double and triple "clicks"
