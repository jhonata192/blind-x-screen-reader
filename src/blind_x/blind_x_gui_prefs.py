...
"""Displays a GUI for the user to set Blind-X preferences."""
...
class BlindXSetupGUI(Gtk.ApplicationWindow):  # pylint: disable=too-many-instance-attributes
    """Preferences window for configuring Blind-X screen reader settings."""

    WINDOW: BlindXSetupGUI | None = None
    ...
    def __init__(self, script: default.Script) -> None:
        if BlindXSetupGUI.WINDOW is not None:
            return
        ...
        self.left_headerbar.get_style_context().add_class("blind-x-left-headerbar")

        self._app_icon = Gtk.Image.new_from_icon_name("blind-x", Gtk.IconSize.MENU)
        ...
        self.panel_headerbar.get_style_context().add_class("blind-x-panel-headerbar")
        ...
        self.sidebar_vbox.get_style_context().add_class("blind-x-sidebar")
    ...
    def show_gui(self) -> None:
        """Show the Blind-X configuration GUI window."""
        ...
        if BlindXSetupGUI.WINDOW is not None:
            BlindXSetupGUI.WINDOW.present()
            return

        BlindXSetupGUI.WINDOW = self

        accel_group = Gtk.AccelGroup()
        BlindXSetupGUI.WINDOW.add_accel_group(accel_group)
        ...
        BlindXSetupGUI.WINDOW.show_all()
        self._sync_headerbar_widths()
        self._set_height_from_sidebar()
        self.stack.set_transition_type(Gtk.StackTransitionType.CROSSFADE)
        self.stack.set_transition_duration(150)
        BlindXSetupGUI.WINDOW.present_with_time(time.time())
    ...
    def apply_button_clicked(self, _widget: Gtk.Button) -> None:
        ...
        blind_x.load_user_settings(self.script, skip_reload_message=True)
    ...
    _BASE_CSS = b"""
        ...
        .blind-x-left-headerbar {
            background-color: @blind_x_sidebar_bg;
            background-image: none;
            border-bottom-width: 0;
        }
        .blind-x-panel-headerbar {
        ...
        .blind-x-sidebar scrolledwindow,
        .blind-x-sidebar list {
            background-color: transparent;
        }
        .blind-x-sidebar list {
            padding: 6px 0;
        }
        .blind-x-sidebar list row {
            border-radius: 9px;
            min-height: 36px;
            padding: 0 8px;
            margin: 0 6px 2px;
        }
        .blind-x-sidebar list row:selected {
            background-color: alpha(@theme_fg_color, 0.10);
        }
        .blind-x-sidebar list row:selected,
        .blind-x-sidebar list row:selected label {
            color: @theme_fg_color;
        }
        .blind-x-sidebar list row:hover {
            background-color: alpha(@theme_fg_color, 0.07);
        }
        .blind-x-sidebar list row:selected:hover {
            background-color: alpha(@theme_fg_color, 0.13);
        }
        .blind-x-sidebar list row:active,
        .blind-x-sidebar list row:selected:active {
    ...
    def _sync_appearance() -> tuple | None:
        ...
        base_provider.load_from_data(BlindXSetupGUI._BASE_CSS)
        ...
        providers.hc.load_from_data(BlindXSetupGUI._HIGH_CONTRAST_CSS)
        dark_css = BlindXSetupGUI._DARK_MODE_CSS
        if "GNOME" in os.environ.get("XDG_CURRENT_DESKTOP", ""):
            dark_css += BlindXSetupGUI._GNOME_DARK_CSS
        ...
        try:
            ...
            if "show-status-shapes" in a11y_settings.list_keys():
                ...
                shapes_provider.load_from_data(BlindXSetupGUI._STATUS_SHAPES_CSS)
                providers.shapes = shapes_provider
            BlindXSetupGUI._apply_appearance(
                interface_settings,
                a11y_settings,
                gtk_settings,
                screen,
    ...
    def suspend_events(self, reason: str = "") -> None:
        """Deregister event listeners that flood Blind-X during UI creation and teardown."""
