...
"""Manager for Blind-X profile creation, loading, and management."""
...
@gsettings_registry.get_registry().gsettings_schema(
    "org.gnome.BlindX.ProfileMetadata",
    name="metadata",
)
class ProfileManager(Extension):
    """Manager for Blind-X profiles."""
    ...
    def load_profile(self, internal_name: str) -> None:
        ...
        blind_x.load_user_settings(skip_reload_message=True)
    ...
    def cycle_settings_profile(
        ...
        blind_x_modifier_manager.get_manager().unset_blind_x_modifiers("Profile changing.")
        command_manager.get_manager().load_keyboard_layout()
        blind_x_modifier_manager.get_manager().refresh_blind_x_modifiers("Profile changed.")
