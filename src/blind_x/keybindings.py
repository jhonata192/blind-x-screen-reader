...
def get_modifier_names(mods: int) -> str:
    ...
    if mods & ORCA_MODIFIER_MASK:
        name = keynames.get_key_name("Blind-X")
        assert name
        text += name + "+"
...
class KeyBinding:
    ...
    def matches(self, keyval: int, keycode: int, modifiers: int) -> bool:
        ...
        # 'H', AltGr: 'period' vs 'periodcentered'), when an Blind-X-modifier
        # command uses a key whose layout maps a different base character
        # (AZERTY: Blind-X+7/8/9 produce Blind-X+'è'/'_'/'ç'), when no non-locking
    ...
    def key_definitions(self, blind_x_modifiers: list[str]) -> list[Atspi.KeyDefinition]:
        """return a list of Atspi key definitions for the given binding."""

        ret = []
        if not self.keycode:
            self.keyval, self.keycode = get_keycodes(self.keysymstring)
        ret.extend(self._create_key_definitions(self.keyval, self.modifiers, blind_x_modifiers))
        ...
        if self.modifiers & SHIFT_MODIFIER_MASK:
            if (upper_keyval := Gdk.keyval_to_upper(self.keyval)) != self.keyval:
                ret.extend(
                    self._create_key_definitions(upper_keyval, self.modifiers, blind_x_modifiers),
                )
        return ret
    ...
    def add_grabs(self, blind_x_modifiers: list[str]) -> None:
        ...
        for kd in self.key_definitions(blind_x_modifiers):
