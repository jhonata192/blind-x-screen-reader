...
"""Provides a D-Bus interface for remotely controlling Blind-X."""
...
class _InterfaceBuilder:
    ...
    def build(cls, registration: _ModuleRegistration) -> type:
        ...
        interface_name = f"org.gnome.BlindX1.{module_name}"
...
@dbus_interface("org.gnome.BlindX1.Service")
class BlindXDBusServiceInterface(Publishable):
    ...
    def ShowPreferences(self) -> bool:  # pylint: disable=invalid-name
        """Shows Blind-X's preferences GUI."""
    ...
    def Quit(self) -> bool:  # pylint: disable=invalid-name
        ...
        from . import blind_x  # pylint: disable=import-outside-toplevel

        # blind_x.shutdown() shuts down the dbus service, so send the response immediately and then
...
class BlindXRemoteController:
    """Manages Blind-X's D-Bus service for remote control."""

    SERVICE_NAME = "org.gnome.BlindX1.Service"
    OBJECT_PATH = "/org/gnome/BlindX1/Service"

    def __init__(self) -> None:
        self._dbus_service_interface: BlindXDBusServiceInterface | None = None
    ...
    def start(self) -> bool:
        ...
        self._dbus_service_interface = BlindXDBusServiceInterface()
        try:
...
_remote_controller: BlindXRemoteController = BlindXRemoteController()


def get_remote_controller() -> BlindXRemoteController:
    """Returns the BlindXRemoteController singleton."""
