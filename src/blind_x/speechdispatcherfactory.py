...
"""Provides an Blind-X speech server for Speech Dispatcher backend."""
...
class SpeechServer(speechserver.SpeechServer):
    """Speech Dispatcher speech server for Blind-X."""
    ...
    def _init(self) -> None:
        try:
            self._client = client = speechd.SSIPClient("Blind-X", component=self._id)
    ...
    def speak(self, text: str | None = None, acss: dict[str, Any] | None = None) -> None:
        ...
        if len(text) == 1:
            if (
                ...
                # See https://github.com/jhonata192/blind-x-screen-reader/issues/657
                self._apply_acss(acss)
    ...
    def speak_character(
        ...
        if not name or name == character:
            if cap_style == CapitalizationStyle.ICON:
                ...
                # speak() does. See https://github.com/jhonata192/blind-x-screen-reader/issues/657
    ...
    def set_output_module(self, module_id: str) -> None:
        ...
        # That might be desired (e.g. self._id impacts what is shown in Blind-X preferences),
