...
class LearnModePresenter(Extension):
    ...
    def handle_event(
        ...
        if event.keyval_name == "F2" and not (
            ...
            self.list_blind_x_shortcuts(script, event)
            return True
    ...
    def list_blind_x_shortcuts(
        self, script: default.Script, event: input_event.KeyboardEvent
    ) -> bool:
        """Shows a simple gui listing Blind-X's bound commands."""
        ...
        title = messages.shortcuts_found_blind_x(items)
    ...
    def show_help(
        ...
        """Displays Blind-X's documentation."""
