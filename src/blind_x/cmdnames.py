...
"""Command names which Blind-X presents in speech and/or braille. These
...
# Translators: Blind-X has a command to synthesize mouse events. This string
# describes the Blind-X command to generate a left mouse button click on the
# current item, typically a widget.
LEFT_CLICK_REVIEW_ITEM = _("Perform left click on current item")

# Translators: Blind-X has a command to synthesize mouse events. This string
# describes the Blind-X command to generate a right mouse button click on the
# current item, typically a widget.
RIGHT_CLICK_REVIEW_ITEM = _("Perform right click on current item")

# Translators: the Blind-X "SayAll" command allows the user to press a key and have
# the entire document in a window be automatically spoken to the user. If the
# user presses any key during a SayAll operation, the speech will be interrupted
# and the cursor will be positioned at the point where the speech was interrupted.
SAY_ALL = _("Speak entire document")

# Translators: the 'flat review' feature of Blind-X allows the user to explore the
# text in a window in a 2D fashion. That is, Blind-X treats all the text from all
# objects in a window (e.g., buttons, labels, etc.) as a sequence of words in a
# sequence of lines. The flat review feature allows the user to explore this text
# by the {previous,next} {line,word,character}. This string is the name of a command
# which causes Blind-X to speak the entire contents of the window using flat review.
SAY_ALL_FLAT_REVIEW = _("Speak entire window using flat review")

# Translators: the "Where Am I" feature of Blind-X allows a user to press a key and
...
# Translators: Blind-X has a command which presents a list with accessible actions
...
# sequence of words in a sequence of lines.  The flat review feature allows
# the user to explore this text by the {previous,next} {line,word,character}.
# This particular command will cause Blind-X to spell the current line character
# by character.
...
# sequence of words in a sequence of lines.  The flat review feature allows
# the user to explore this text by the {previous,next} {line,word,character}.
# This particular command will cause Blind-X to spell the current word or item
...
# sequence of words in a sequence of lines.  The flat review feature allows
# the user to explore this text by the {previous,next} {line,word,character}.
# This particular command will cause Blind-X to present the character phonetically,
...
# sequence of words in a sequence of lines.  The flat review feature allows
# the user to explore this text by the {previous,next} {line,word,character}.
# This particular command will cause Blind-X to present the character's unicode
# value.
...
# on the display (typically 40 cells).  Blind-X provides the feature to build up a
...
# Translators: Blind-X allows the user to turn speech synthesis on or off.
...
# Translators: Blind-X supports two keyboard layouts: a "desktop" layout intended
...
# users to set their preferences for Blind-X.
SHOW_PREFERENCES_GUI = _("Display the preferences configuration dialog")

# Translators: the preferences configuration dialog is the dialog that allows
# users to set their preferences for a specific application within Blind-X.
SHOW_APP_PREFERENCES_GUI = _("Display the application preferences configuration dialog")

# Translators: Blind-X allows the user to enable/disable spoken indentation.
...
# three"). This string to be translated refers to an Blind-X command for quickly
# toggling between the two options.
CHANGE_NUMBER_STYLE = _("Change spoken number style")

# Translators: Blind-X has a command to present the current clipboard contents to
...
# Translators: Blind-X allows users to cycle through punctuation levels. None,
# some, most, or all, punctuation will be spoken.
CYCLE_PUNCTUATION_LEVEL = _("Cycle to the next speaking of punctuation level")

# Translators: Blind-X allows users to cycle through the speech synthesizers
...
# Translators: Blind-X has a feature whereby users can set up different "profiles,"
# which are collection of settings which apply to a given task, such as a
# "Spanish" profile which would use Spanish text-to-speech and Spanish braille
# and selected when reading Spanish content. This string to be translated refers
# to an Blind-X command which makes it possible for users to quickly cycle amongst
# their saved profiles without having to get into a GUI.
CYCLE_SETTINGS_PROFILE = _("Cycle to the next settings profile")

# Translators: Blind-X has a feature that allows users to save and load collections
# of settings into named "profiles". This string to be translated refers to an
# Blind-X command which presents the name of the currently active profile.
PRESENT_CURRENT_PROFILE = _("Present the current profile")

# Translators: Blind-X uses Speech Dispatcher to present content to users via text-
# to-speech. Speech Dispatcher has a feature to control how capital letters are
# presented: Do nothing at all, say the word 'capital' prior to presenting a
# capital letter, or play a tone which Speech Dispatcher refers to as a sound
# 'icon'. This string to be translated refers to an Blind-X command which makes it
...
# Translators: When the user is navigating through text, Blind-X can optionally
...
# Translators: this is a debug message that Blind-X users will not normally see. It
# describes a debug routine that allows the user to adjust the level of debug
# information that Blind-X generates at run time.
...
# information which impacts Blind-X's logic and/or presentation to users.
DEBUG_CLEAR_ATSPI_CACHE_FOR_APPLICATION = _("Clear the AT-SPI cache for the current application")

# Translators: Blind-X allows the item under the pointer to be spoken. This toggles
# the feature without the need to get into a GUI.
MOUSE_REVIEW_TOGGLE = _("Toggle mouse review mode")

# Translators: Blind-X has a command to present the battery status (e.g. level, whether
# or not it is plugged in, etc.). This string is the name of that command.
PRESENT_BATTERY_STATUS = _("Present battery status")

# Translators: Blind-X has a command to present the CPU and memory usage as percents.
# This string is the name of that command.
PRESENT_CPU_AND_MEMORY_USAGE = _("Present CPU and memory usage")

# Translators: Blind-X has a command to present the current time in speech and in
# braille.
PRESENT_CURRENT_TIME = _("Present current time")

# Translators: Blind-X has a command to present the current date in speech and in
# braille.
PRESENT_CURRENT_DATE = _("Present current date")

# Translators: Blind-X has a command to present the state of modifier keys such as
# Caps Lock and Num Lock (e.g. "caps lock on. num lock off.").
PRESENT_MODIFIER_KEYS_STATE = _("Present the state of modifier keys")

# Translators: Blind-X has a command to present the pixel size and location of
# the current object. This string is how this command is described in the list
# of keyboard shortcuts.
PRESENT_SIZE_AND_POSITION = _("Present size and location of current object")

# Translators: This command toggles all (other) Blind-X commands so that the
...
# would cause the Blind-X command to be used. This would mean Alt+Down could not
# be used in editors to move the current line of text down. By temporarily
# disabling Blind-X commands, Alt+Down would be ignored by Blind-X and work as
# expected in the editor.
BYPASS_MODE_TOGGLE = _("Toggle all Blind-X command keys")
...
# Translators: Blind-X has a command to review chat room messages in
# speech and braille. This string to be translated is associated with the
...
# Translators: this is a command which causes Blind-X to present the last received
# notification message.
NOTIFICATION_MESSAGES_LAST = _("Present last notification message")

# Translators: this is a command which causes Blind-X to present a list of all the
# notification messages received.
NOTIFICATION_MESSAGES_LIST = _("Present notification messages list")

# Translators: this is a command which causes Blind-X to present the previous
# notification message.
NOTIFICATION_MESSAGES_PREVIOUS = _("Present previous notification message")

# Translators: this is a command which causes Blind-X to present the next
# notification message.
...
# by Blind-X's caret navigation to be expanded.
...
# Translators: A live region is an area of a web page that is periodically
# updated, e.g. stock ticker. https://w3c.github.io/aria/#dfn-live-region
# This string refers to an Blind-X command which allows the user to toggle whether
# or not Blind-X pays attention to changes in live regions. Note that turning off
# monitoring of live events is NOT the same as turning the politeness level
# to "off". The user can opt to have no notifications presented (politeness
# level of "off") and still manually review recent updates to live regions via
# Blind-X commands for doing so -- as long as the monitoring of live regions is
# enabled.
...
# Translators: This string refers to an Blind-X command. The command causes Blind-X
...
# Translators: Blind-X has a modal math navigation mode powered by MathCAT
...
# both for presentation and navigation. This string is associated with the Blind-X
...
# This string is associated with the Blind-X command to manually switch
...
# This string is associated with the Blind-X command to enable sticky focus mode.
...
# This string is associated with the Blind-X command to enable sticky browse mode.
