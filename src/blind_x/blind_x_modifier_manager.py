...
"""Manages the Blind-X modifier key."""
...
class BlindXModifierManager:
    """Manages the Blind-X modifier."""
    ...
    def _on_device_changed(self, _device_manager, device: Gdk.Device) -> None:
        ...
        if source == Gdk.InputSource.KEYBOARD:
            self.refresh_blind_x_modifiers("Keyboard change detected.")

    def is_blind_x_modifier(self, modifier: str) -> bool:
        """Returns True if modifier is one of the user's Blind-X modifier keys."""

        if modifier not in self.get_blind_x_modifier_keys():
            return False
    ...
    def get_pressed_state(self) -> bool:
        """Returns True if the Blind-X modifier has been pressed but not yet released."""
    ...
    def add_grabs_for_blind_x_modifiers(self) -> None:
        """Adds grabs for all of the user's Blind-X modifier keys."""

        for modifier in self.get_blind_x_modifier_keys():
            if modifier in ["Insert", "KP_Insert"]:
                self.add_modifier_grab(modifier)

    def remove_grabs_for_blind_x_modifiers(self) -> None:
        """Removes grabs for all of the user's Blind-X modifier keys."""
    ...
    def get_blind_x_modifier_keys(self) -> list[str]:
        """Returns the active Blind-X modifier keys via override or layered lookup."""
    ...
    def _lookup_modifier_keys(self) -> list[str]:
        ...
        layout = registry.layered_lookup(
            ...
            "keyboard-layout",
            "",
            genum="org.gnome.BlindX.KeyboardLayout",
            default="desktop",
        )
        if layout == "desktop":
    ...
    def needs_modifier_refresh(self) -> bool:
        ...
        return self.get_blind_x_modifier_keys() != self._applied_modifier_keys

    def set_modifiers_for_layout(self) -> None:
        """Unsets and refreshes modifier keys for the current layout."""

        self.unset_blind_x_modifiers("Keyboard layout changing.")
        self.refresh_blind_x_modifiers("Keyboard layout changed.")

    def refresh_blind_x_modifiers(self, reason: str = "") -> None:
        """Refreshes the Blind-X modifier keys, including grabs and xmodmap."""

        msg = "ORCA MODIFIER MANAGER: Refreshing Blind-X modifiers"
        if reason:
            msg += f": {reason}"
        debug.print_message(debug.LEVEL_INFO, msg, True)
        ...
        for modifier in list(self._grabbed_modifiers.keys()):
            self.remove_modifier_grab(modifier)
        self._is_pressed = False
        self._applied_modifier_keys = list(self.get_blind_x_modifier_keys())
        self.add_grabs_for_blind_x_modifiers()
        ...
        self._create_blind_x_xmodmap()

    def _create_blind_x_xmodmap(self) -> None:
        """Makes an Blind-X-specific Xmodmap so that the Blind-X modifier works."""

        msg = "ORCA MODIFIER MANAGER: Creating Blind-X xmodmap"
        debug.print_message(debug.LEVEL_INFO, msg, True)

        blind_x_modifiers = self.get_blind_x_modifier_keys()
        if "Caps_Lock" in blind_x_modifiers or "Shift_Lock" in blind_x_modifiers:
            self.set_caps_lock_as_blind_x_modifier(True)
            self._caps_lock_cleared = True
        elif self._caps_lock_cleared:
            self.set_caps_lock_as_blind_x_modifier(False)
            self._caps_lock_cleared = False

    def unset_blind_x_modifiers(self, reason: str = "") -> None:
        """Turns the Blind-X modifiers back into their original purpose."""

        msg = "ORCA MODIFIER MANAGER: Unsetting Blind-X modifiers"
        if reason:
            msg += f": {reason}"
        debug.print_message(debug.LEVEL_INFO, msg, True)
    ...
    def set_caps_lock_as_blind_x_modifier(self, enable: bool) -> None:
        """Enable or disable use of the caps lock key as an Blind-X modifier key."""

        msg = "ORCA MODIFIER MANAGER: Setting caps lock as the Blind-X modifier"
        debug.print_message(debug.LEVEL_INFO, msg, True)
...
_manager: BlindXModifierManager = BlindXModifierManager()


def get_manager() -> BlindXModifierManager:
    """Returns the BlindXModifierManager singleton."""
