...
#
# Generate markdown documentation for Blind-X's GSettings schemas.
#
...
def generate_documentation(
    ...
    lines: list[str] = []
    lines.append("# Blind-X GSettings Schemas Reference")
    lines.append("")
    lines.append("[TOC]")
    lines.append("")
    lines.append("## Overview")
    lines.append("")
    lines.append("Blind-X stores settings under `/org/gnome/blind-x/`.")
    lines.append("- Profile-level settings: `/org/gnome/blind-x/<profile>/<schema-name>/`")
    lines.append("- App-specific overrides: `/org/gnome/blind-x/<profile>/apps/<app>/<schema-name>/`")
    lines.append(
        "- Voice settings: `/org/gnome/blind-x/<profile>/voice-sets/<voice-set>/<voice-type>/`"
    )
    lines.append(
        "- App-specific voice overrides: "
        "`/org/gnome/blind-x/<profile>/apps/<app>/voice-sets/<voice-set>/<voice-type>/`"
    )
    lines.append("- Voice settings (deprecated): `/org/gnome/blind-x/<profile>/voices/<voice-type>/`")
    lines.append(
        "- App-specific voice overrides (deprecated): "
        "`/org/gnome/blind-x/<profile>/apps/<app>/voices/<voice-type>/`"
    )
    lines.append("")
    ...
    lines.append("- `<schema-name>`: Blind-X schema name, e.g. `typing-echo`, `speech`, `braille`.")
    ...
    lines.append("")
    lines.append(
        "When Blind-X reads a setting, it checks several layers from most specific to least specific:"
    )
    lines.append("")
    lines.append(
    ...
    lines.append("")
    lines.append("You can read and write individual Blind-X settings with `dconf`.")
    lines.append("")
    lines.append("- `dconf list /org/gnome/blind-x/`: list profiles")
    lines.append("- `dconf read /org/gnome/blind-x/default/speech/enable`: read a single key")
    lines.append("- `dconf write /org/gnome/blind-x/default/speech/enable false`: write a single key")
    lines.append("")
    lines.append(
        "`gsettings` also works but requires both the schema ID and path, since Blind-X "
        "uses relocatable schemas:"
    )
    lines.append("")
    lines.append("- `gsettings get org.gnome.BlindX.Speech:/org/gnome/blind-x/default/speech/ enable`")
    lines.append(
        "- `gsettings set org.gnome.BlindX.Speech:/org/gnome/blind-x/default/speech/ enable false`"
    )
    lines.append("")
    ...
    lines.append("")
    lines.append("- `dconf watch /org/gnome/blind-x/`: all Blind-X changes")
    lines.append("- `dconf watch /org/gnome/blind-x/default/speech/`: one schema path")
    lines.append("- `gsettings monitor org.gnome.BlindX.Speech:/org/gnome/blind-x/default/speech/`")
    lines.append("")
    lines.append("## Migrating from JSON Settings")
    lines.append("")
    lines.append(
        "Blind-X v50 automatically migrated JSON settings from `~/.local/share/blind-x/` "
        "into dconf on first launch. Users upgrading from Blind-X v49 or earlier directly "
        "to v51 or later can use the standalone import tool:"
    )
    lines.append("")
    lines.append("```sh")
    lines.append("python tools/gsettings_import_json.py import ~/.local/share/blind-x")
    lines.append("")
    lines.append(
        "Use `--prefix <blind-x-prefix>` if schemas are installed in a non-default prefix."
    )
    lines.append("")
    lines.append("## Transferring, Backing Up, and Restoring Settings")
    lines.append("")
    lines.append(
        "All Blind-X settings live in dconf under `/org/gnome/blind-x/`. You can dump them "
    ...
    lines.append("")
    lines.append("- Dump all settings to a file: `dconf dump /org/gnome/blind-x/ > blind-x-settings.ini`")
    lines.append("- Load settings from a file: `dconf load /org/gnome/blind-x/ < blind-x-settings.ini`")
    lines.append("- Reset all settings to defaults: `dconf reset -f /org/gnome/blind-x/`")
    lines.append("")
    lines.append(
        ...
        "`dconf reset -f /org/gnome/blind-x/ && dconf load /org/gnome/blind-x/ < blind-x-settings.ini`"
    )
    lines.append("")
    lines.append("These operations also work at the profile level:")
    lines.append("")
    lines.append("- Dump one profile: `dconf dump /org/gnome/blind-x/default/ > default-profile.ini`")
    lines.append("- Load one profile: `dconf load /org/gnome/blind-x/default/ < default-profile.ini`")
    lines.append("- Reset one profile to start fresh: `dconf reset -f /org/gnome/blind-x/default/`")
    lines.append("")
    lines.append("---")
    lines.append("")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Generate Blind-X GSettings markdown docs")
    parser.add_argument(
        "src_dir",
        nargs="?",
