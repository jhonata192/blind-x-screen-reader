...
# profile_orca.py
#
# Launches Blind-X under cProfile so you can interact with it normally,
...
"""Profiles a running Blind-X session under cProfile."""
...
def _short_path(filename: str) -> str:
    """Shortens a path to the blind_x-relative portion."""

    if "/blind_x/" in filename:
        return "blind_x/" + filename.split("/blind_x/", 1)[1]
    return filename
...
def _print_blind_x_stats(all_stats: dict) -> None:
    """Prints the top blind_x functions by total time."""

    sorted_keys = [key for key in all_stats if "/blind_x/" in key[0]]
...
def _find_key(all_stats: dict, name: str) -> tuple | None:
    ...
    matches = [key for key in all_stats if key[2] == name and "/blind_x/" in key[0]]
    if not matches:
        return None
...
def main() -> None:
    """Launches Blind-X under cProfile and prints the results."""

    import argparse

    parser = argparse.ArgumentParser(description="Profile a running Blind-X session.")
    parser.add_argument(
        "--target",
        metavar="FUNCTION",
    ...
    blind_x_bin = shutil.which("blind-x")
    if blind_x_bin is None:
        print("blind-x not found in PATH.", file=sys.stderr)
        sys.exit(1)

    print("Launching Blind-X under cProfile (debug output suppressed).")
    print("Interact with Blind-X normally, then Ctrl+C when done.\n")

    os.environ["ORCA_PROFILING"] = "1"

    sys.argv = [blind_x_bin, "--replace"]

    profiler = cProfile.Profile()
    profiler.enable()
    try:
        runpy.run_path(blind_x_bin, run_name="__main__")
    ...
    if args.target:
        ...
        print(f"\n{'=' * 72}")
        print(f"Top {_TOP_N} functions by total time (blind_x only)")
        print(f"{'=' * 72}\n")
        _print_blind_x_stats(all_stats)
