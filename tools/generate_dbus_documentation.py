...
#
# Generate markdown documentation for Blind-X's D-Bus remote controller.
#
...
SERVICE_NAME = "org.gnome.BlindX1.Service"
SERVICE_PATH = "/org/gnome/BlindX1/Service"
SERVICE_INTERFACE = "org.gnome.BlindX1.Service"
INTERFACE_PREFIX = "org.gnome.BlindX1"
...
def find_modules(blind_x_src_dir):
    """Yields (class_name, commands, parameterized, getters, setters) per D-Bus module."""

    for path in sorted(Path(blind_x_src_dir).glob("*.py")):
        try:
...
def _is_service_interface_class(class_node):
    """Returns True if class_node is decorated with @dbus_interface('org.gnome.BlindX1.Service')."""
...
def _uses_meta_protocol(blind_x_src_dir):
    """Returns True if the meta-protocol marker class is present in dbus_service.py."""

    path = Path(blind_x_src_dir) / DBUS_SERVICE_FILE
    tree = ast.parse(path.read_text(encoding="utf-8"))
    return any(
...
def find_service_methods(blind_x_src_dir):
...
def _format_module_section(name, commands, parameterized, getters, setters, *, meta_protocol):
    ...
    if commands:
        ...
        if meta_protocol:
            lines += [
                "**Method:** `org.gnome.BlindX.Module.ExecuteCommand`",
                "",
    ...
    if parameterized:
        ...
        if meta_protocol:
            lines += [
                "**Method:** `org.gnome.BlindX.Module.ExecuteParameterizedCommand`",
                "",
            ]
    ...
    if getters or setters:
        if meta_protocol:
            lines += [
                "#### Settings",
                "",
                "**Methods:** `org.gnome.BlindX.Module.ExecuteRuntimeGetter` / "
                "`org.gnome.BlindX.Module.ExecuteRuntimeSetter`",
                "",
...
def generate_documentation(blind_x_src_dir):
    """Walks the source tree and renders the service interface plus each registered module."""

    meta_protocol = _uses_meta_protocol(blind_x_src_dir)

    header = [
        "# Blind-X D-Bus Service Commands Reference",
        "",
    ...
    sections.append(
        _format_service_section(find_service_methods(blind_x_src_dir), meta_protocol=meta_protocol)
    )
    sections.append("---")
    sections.append("")
    ...
    if meta_protocol:
        ...
        sections.append("")
    for entry in sorted(find_modules(blind_x_src_dir), key=lambda e: e[0]):
...
def main():
    ...
    src_dir = os.path.join(parent_dir, "src", "blind-x")
