...
#
# Standalone tool for importing Blind-X JSON settings into GSettings/dconf.
#
# Examples (run from the repo root):
#
# Import your JSON settings into dconf. First preview what will be written:
#
#      python tools/gsettings_import_json.py import --dry-run $HOME/.local/share/blind-x
#
#    If the output looks correct, run the import for real. WARNING: This overwrites
#    any existing Blind-X settings in your dconf database. To clear imported settings
#    afterwards use "dconf reset -f /org/gnome/blind-x/" but note that this will not
#    restore whatever was there before the import.
#
#      python tools/gsettings_import_json.py import $HOME/.local/share/blind-x
#
#    View the result:
#
#      dconf dump /org/gnome/blind-x/
#
#
...
GSETTINGS_PATH_PREFIX = "/org/gnome/blind-x/"
    ...
    def get_settings(self, schema_id: str, path: str) -> Gio.Settings:
        ...
        if schema is None:
            print(
                ...
                "directories. \nIf you built Blind-X with a custom prefix "
...
def main() -> None:
    """CLI entry point for importing Blind-X JSON settings into GSettings/dconf."""
    parser = argparse.ArgumentParser(description="Blind-X GSettings tool: import JSON settings")
    parser.add_argument(
        "--prefix",
        help="Blind-X install prefix (e.g. /usr/local). Adds <prefix>/share to XDG_DATA_DIRS.",
    )
